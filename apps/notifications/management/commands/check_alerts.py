...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        stale_steps = ApprovalNode.objects.filter(
            status='pending',
        ...
        for step in stale_steps:
            ...
            if escalate_enabled and not step.escalated_at:
                ...
                if higher_approver:
                    ...
                    count += 1
                    self.stdout.write(f'  [审批升级] {flow_title}: {old_approver.username} -> {higher_approver.username}')

        # ── 3. 审批积压预警 ─────────────────────────────────────────────────
        # 同一人待审批 > 5 笔，每12小时通知一次（用 Notification.created_at 本身做去重）
        from django.db.models import Q
        approver_counts = ApprovalNode.objects.filter(
            status='pending'
        ).values('approver').annotate(
        ...
        stale_wages = WageRecord.objects.filter(
            status='approved',
            updated_at__lt=dj_timezone.now() - timedelta(days=7)
        ).select_related('company')
        ...
        self.stdout.write(
            self.style.SUCCESS(f'[{datetime.now()}] 预警检测完成，共发送 {count} 条通知')
        )
