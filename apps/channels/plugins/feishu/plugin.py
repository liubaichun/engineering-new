...
class FeishuPlugin(BaseChannelPlugin):
            ...
            'content': json.dumps(
                {
                    'card': {
                        ...
                        'elements': [
                            ...
                            {'tag': 'hr'},
                            {'tag': 'note', 'elements': [{'tag': 'plain_text', 'content': '来自企业信息化管理系统'}]},
                        ],
                    }
                },
                ensure_ascii=False,
    ...
    def _send_webhook_message(self, title: str, content: str) -> tuple[bool, str]:
        ...
        if secret:
            ...
            sign = hmac.new(sign_str.encode('utf-8'), sign_str.encode('utf-8'), digestmod=hashlib.sha256).hexdigest()
        ...
        payload = {
            ...
            'card': {
                ...
                'elements': [
                    {'tag': 'div', 'text': {'tag': 'lark_md', 'content': content}},
                    {'tag': 'hr'},
                    {'tag': 'note', 'elements': [{'tag': 'plain_text', 'content': '企业信息化管理系统 · 自动化通知'}]},
                ],
            },
        }
