...
def _mkpdf(wage_record):
    ...
    for l1, v1, l2, v2 in rows_info:
        pdf.set_x(10)
        pdf.set_text_color(*_GREY); pdf.cell(20, 7, l1)
        pdf.set_text_color(*_BLACK); pdf.cell(75, 7, str(v1)[:28])
        pdf.set_text_color(*_GREY); pdf.cell(20, 7, l2)
        pdf.set_text_color(*_BLACK); pdf.cell(75, 7, str(v2)[:28])
        pdf.ln(7)

    # 实发高亮行
    pdf.set_fill_color(*_LGREY)
    pdf.set_x(10)
    pdf.set_text_color(*_GREY); pdf.cell(20, 7, '实发工资')
    pdf.set_text_color(*_RED);  pdf.set_font('NotoSC', 'B', 11)
    pdf.cell(75, 7, f(net))
    pdf.set_text_color(*_GREY); pdf.cell(20, 7, '工资期间')
    pdf.set_text_color(*_BLACK); pdf.set_font('NotoSC', '', 9)
    ...
    def two_col_row(items):
        ...
        for i in range(0, len(items), 2):
            pdf.set_x(10)
            l1, v1 = items[i]
            pdf.set_text_color(*_GREY); pdf.set_font('NotoSC', '', 8)
            pdf.cell(57, 6, f'  {l1}')
            pdf.set_text_color(*_BLACK); pdf.set_font('NotoSC', '', 8)
    ...
    def total_bar(label, value_str, bg, fg=_BLACK, fs=10):
        ...
        pdf.set_x(10)
        pdf.set_text_color(*fg); pdf.set_font('NotoSC', 'B', fs)
