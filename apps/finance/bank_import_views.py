...
TX_TYPE_RULES = {
    ...
    '批量代发付费': {'direction': 'expense', 'category': 'skip', 'note': '银行批量代发手续费，直接跳过'},
    '跨行汇款-普通': {
        'direction': 'expense',
        'category': '金融服务',
        'cp_override': '银行服务费',
    ...
    '行内汇款-普通': {'direction': 'expense', 'category': 'skip', 'note': '行内转账，手续费可忽略'},
    '银承到期扣款': {'direction': 'expense', 'category': '金融服务', 'note': '银承到期'},
    '贷款发放': {'direction': 'expense', 'category': '借款还款', 'note': '贷款发放'},
    '贷款归还': {'direction': 'expense', 'category': '借款还款', 'note': '贷款归还'},
    # 收入类
    '账户结息': {
        'direction': 'income',
        'category': '利息收入',
    ...
    'IBPS对公提回贷记': {'direction': 'income', 'category': 'auto', 'note': '看摘要关键词决定最终类别'},
    'IBPS对私提回贷记': {'direction': 'income', 'category': 'auto', 'note': '看摘要关键词决定最终类别'},
    # 行内转账收方
    '行内汇款入账': {'direction': 'income', 'category': 'auto', 'note': '看摘要关键词决定最终类别'},
}
...
def _classify(t: ParsedTransaction) -> dict:
    ...
    if rule:
        ...
        if category == '往来':
            is往来 = True
            往来_type, 往来_remark = _detect_往来_subtype(cp_name, summary, tx_type, t)
        else:
            往来_type, 往来_remark = '', ''
        ...
        return dict(
            direction=direction,
            category=category,
            counterparty_override=cp_override,
            skip_cp=skip_cp,
            cp_type=cp_type,
            is_skip_record=is_skip,
            is往来=is往来,
            往来_type=往来_type,
            往来_remark=往来_remark,
        )
...
def _upsert_counterparty(company, cp_name: str, cp_account: str, cp_bank: str, cp_type: str, direction: str):
    ...
    try:
        if direction == 'income':
            # 对方是客户 → 写入Client
            client, created = Client.objects.get_or_create(company=company, name=name, defaults=base_fields)
            ...
            if Supplier.objects.filter(company=company, name=name).exists():
                Client.objects.get_or_create(company=company, name=name, defaults=base_fields)
            return client
        else:
            # 对方是供应商 → 写入Supplier
            supplier, created = Supplier.objects.get_or_create(company=company, name=name, defaults=base_fields)
            if bank_updates and created:
                for k, v in bank_updates.items():
            ...
            if Client.objects.filter(company=company, name=name).exists():
                Supplier.objects.get_or_create(company=company, name=name, defaults=base_fields)
            return supplier
    except Exception:
...
def _reconcile_invoice(
    bs, cp_name, amount, tx_date, pending_income_invoices, pending_expense_invoices, date_tolerance=5
):
    """
    ...
    candidates = pending_income_invoices if bs.direction == 'income' else pending_expense_invoices
...
def list_banks(request):
    banks = [{'code': cls().bank_code, 'name': cls().bank_name} for cls in ALL_ADAPTERS]
...
def preview_bank_statement(request):
    ...
    if 'application/json' in content_type or (
        ...
        try:
            ...
            with open('/tmp/bank_debug.log', 'a') as f:
                f.write(
                    f'raw_b64_len={len(raw_b64)} first10={raw_b64[:10]} decoded_len={len(content)} first_bytes={content[:20].hex()}\n'
                )
    ...
    if bank_account_id_raw is not None and bank_account_id_raw != '' and str(bank_account_id_raw).isdigit():
        ...
        if (
            detected_bank
            and detected_bank != bank_account.bank_code
            and detected_bank != ('PINGAN' if bank_account.bank_code == 'PA' else bank_account.bank_code)
        ):
            bank_display_map = {
                'CMB': '招商银行',
                'ICBC': '工商银行',
    ...
    try:
        ...
        if validate_bank_code:
            ...
            _debug_log('[preview] calling detect_and_parse')
...
def confirm_bank_import(request):
    ...
    try:
        ...
        with open(LOG_PATH, 'w') as _lf:
            _lf.write('===== confirm_bank_import START =====\n')
        ...
        for idx, row in enumerate(rows):
            with open(LOG_PATH, 'a') as _lf:
                _lf.write(
                    f'[{idx}] date={row.get("transaction_date", "")} serial={row.get("bank_serial", "")!r} '
            ...
            if isinstance(t_date, str) and t_date:
                tx_date = datetime.datetime.strptime(
                    t_date.split('T')[0] if 'T' in t_date else t_date, '%Y-%m-%d'
                ).date()
            else:
            ...
            if BankStatement.objects.filter(
                company=company, transaction_date=tx_date, amount=amount, balance=bal, counterparty_account=cp_account
            ).exists():
                with open(LOG_PATH, 'a') as _lf:
            ...
            try:
                ...
                with open(LOG_PATH, 'a') as _lf:
                    _lf.write(
                        f'  >>> OK idx={idx} bs_id={bs.id} inc_id={inc_obj.id if inc_obj else None} '
            ...
            if bs and amount and cp_name:
                try:
                    _reconcile_invoice(
                        ...
                        amount,
                        tx_date,
                        pending_income_invoices,
                        pending_expense_invoices,
                        date_tolerance=5,
                    )
        ...
        # ── 汇总写入 ───────────────────────────────────────────────────
        with open(LOG_PATH, 'a') as _lf:
            _lf.write('===== confirm_bank_import END =====\n')
            _lf.write(
                f'imported_income={income_count} imported_expense={expense_count} '
