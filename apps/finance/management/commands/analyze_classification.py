...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for e in Expense.objects.filter(expense_type='finance_expense').order_by('-amount'):
            ...
            self.stdout.write(
                f'¥{e.amount:>10,.2f} | desc=[{desc[:40]}] | bs=[{bs_summary[:50]}] | {supplier[:20]:20s} | {company[:10]}'
            )

        self.stdout.write('\n=== admin_expense(非报销) 前30条 ===')
        for e in (
            Expense.objects.filter(expense_type='admin_expense')
            .exclude(description__contains='报销')
            .order_by('-amount')[:30]
        ):
            desc = e.description or ''
            supplier = e.supplier or ''
            ...
            bs_summary = bs.summary if bs else ''
            self.stdout.write(
                f'¥{e.amount:>10,.2f} | desc=[{desc[:40]}] | bs=[{bs_summary[:50]}] | {supplier[:25]:25s} | {company[:10]}'
            )
