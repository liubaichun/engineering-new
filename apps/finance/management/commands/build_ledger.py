...
from apps.finance.models import Income, Expense, Company, RelatedPartyLedger
...
def build_ledger():
    ...
    for le in lend_expenses:
        entries.append(RelatedPartyLedger(
    ...
    for inc in Income.objects.filter(
        income_category__in=['internal_transfer', 'equity']
    ).select_related('company').order_by('date', 'id'):
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # 按公司+方向汇总
        summary = RelatedPartyLedger.objects.values('company__name', 'direction')\
            .annotate(cnt=Count('id'), total=Sum('amount'))\
            .order_by('company__name', 'direction')
        
        self.stdout.write('\n=== 往来台账汇总 ===')
        for row in summary:
            self.stdout.write(f'  {row["company__name"]:15s} | {row["direction"]:10s} | {row["cnt"]:>3}笔 | ¥{float(row["total"]):>10.2f}')
        
        # 按对手方统计未结清余额
        active = RelatedPartyLedger.objects.filter(status='active')\
        ...
        self.stdout.write('\n=== 未结清往来（谁还欠谁钱）===')
        for row in active:
            self.stdout.write(f'  {row["company__name"]:15s} ← {row["counterparty"]:25s} 还欠 ¥{float(row["total"]):>10.2f}')
        
        # 已结清
        settled = RelatedPartyLedger.objects.filter(status='settled')\
            .values('company__name', 'counterparty')\
        ...
        for row in settled[:20]:
            self.stdout.write(f'  {row["company__name"]:15s} ↔ {row["counterparty"]:25s} 共 ¥{float(row["total"]):>10.2f}')
