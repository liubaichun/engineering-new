...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        companies = [Company.objects.get(id=target_id)] if target_id else Company.objects.all()
        ...
        for co in companies:
            ...
            inc_qs = Income.objects.filter(company=co).filter(
                # 以下两个条件满足任一即需要处理
                customer=''
            )
            inc_filled = 0
            for inc in inc_qs:
                bs = (
                    BankStatement.objects.filter(company=co, matched_income=inc)
                    .exclude(counterparty_name__isnull=True)
            ...
            # ── 反填 Expense.supplier ───────────────────────────────────
            exp_qs = Expense.objects.filter(company=co, supplier='')
            exp_filled = 0
            for exp in exp_qs:
                bs = (
                    BankStatement.objects.filter(company=co, matched_expense=exp)
                    .exclude(counterparty_name__isnull=True)
                    .exclude(counterparty_name='')
                    .first()
                )
