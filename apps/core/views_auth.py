...
此链接30分钟内有效。"""
                ...
                return Response({'status': 'success', 'message': '重置链接已发送到您的邮箱，请查收。'})
            except Exception as e:
                return Response(
                    {'status': 'error', 'message': f'邮件发送失败: {str(e)}'},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR,
                )

        return Response({'status': 'success', 'message': '如果该邮箱已注册，重置链接已发送。'})
...
class PasswordResetConfirmView(APIView):
    ...
    def post(self, request: Request, uidb64: str, token: str) -> Response:
        ...
        if not default_token_generator.check_token(user, token):
            return Response(
                {'status': 'error', 'message': '链接已过期，请重新申请'}, status=status.HTTP_400_BAD_REQUEST
            )
...
class LoginView(APIView):
    ...
    def post(self, request: Request) -> HttpResponse | JsonResponse:
        ...
        return Response(
            {'status': 'error', 'message': '登录失败', 'errors': errors}, status=status.HTTP_400_BAD_REQUEST
        )
...
class ChangePasswordView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        if not old_password or not new_password:
            return Response(
                {'status': 'error', 'message': '旧密码和新密码都不能为空'}, status=status.HTTP_400_BAD_REQUEST
            )

        if confirm_password and new_password != confirm_password:
            return Response(
                {'status': 'error', 'message': '两次输入的新密码不一致'}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not any(c.islower() for c in pwd):
            return Response(
                {'status': 'error', 'message': '密码必须包含至少一个小写字母'}, status=status.HTTP_400_BAD_REQUEST
            )
        if not any(c.isdigit() for c in pwd):
            return Response(
                {'status': 'error', 'message': '密码必须包含至少一个数字'}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response(
                {'status': 'error', 'message': f'密码修改失败：{str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class CurrentUserView(APIView):
    ...
    def put(self, request: Request) -> Response:
        ...
        if serializer.is_valid():
            serializer.save()
            return Response(
                {'status': 'success', 'message': '更新成功', 'user': serializer.data}, status=status.HTTP_200_OK
            )
        return Response(
    ...
    def post(self, request: Request) -> Response:
        ...
        return Response(
            {
                'status': 'error',
                'message': '不支持的操作。使用 action=switch_company+company_id 或 action=my_companies',
            },
