        ...
        for row_idx, row_data in enumerate(rows, start=hdr_row + 1):
            ...
            for col_idx, (raw_val, col_type) in enumerate(zip(row_data, col_types), start=1):
                ...
                if col_type == 'money':
                    ...
                    cell.alignment = CENTER
                    cell.font = Font(name='Microsoft YaHei', size=10, color=_status_color(str(raw_val)))
                elif col_type == 'bool':
                    cell.alignment = CENTER
                    cell.font = Font(name='Microsoft YaHei', size=10, color='166534' if raw_val else '991b1b')
                else:
                    cell.alignment = LEFT
                    cell.font = NORMAL_FONT
...
def _format_value(val, col_type):
    ...
    if col_type == 'date':
        ...
        # 字符串日期
        for fmt in ('%Y-%m-%d', '%Y/%m/%d', '%Y-%m-%d %H:%M:%S', '%Y-%m-%dT%H:%M:%S', '%d/%m/%Y', '%m/%d/%Y'):
            try:
...
def export_wage_records(records, company_name=''):
    ...
    for i, r in enumerate(records, 1):
        rows.append(
            [
                i,
                r.employee_company.employee.name
                if r.employee_company and r.employee_company.employee
                else (r.employee.name if r.employee else r.employee_name or ''),
                r.employee_company.employee.id_card
                if r.employee_company and r.employee_company.employee
                else (r.employee.id_card if r.employee else ''),
                r.company.name if r.company else '',
                r.department or '',
                r.position or '',
                r.base_salary or 0,
                r.position_salary or 0,
                r.overtime_pay or 0,
                r.bonus or 0,
                r.social_insurance or 0,
                r.housing_fund or 0,
                r.tax or 0,
                r.net_salary or 0,
    ...
    wb_sheets = [{'title': title, 'headers': headers, 'rows': rows, 'column_types': col_types, 'freeze': 'A3'}]
...
def export_income_records(records):
    ...
    return export_to_xlsx(
        [{'title': title, 'headers': headers, 'rows': rows, 'column_types': col_types, 'freeze': 'A3'}],
...
def export_invoices(records):
    ...
    rows = [
        [
            ...
            r.tax_amount or 0,
            (r.amount or 0) + (r.tax_amount or 0),
