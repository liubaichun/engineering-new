...
def _send_to_binding_channel(binding, subject, content_str):
    ...
    try:
        ...
        logger.info(f'[Notify] Sent via {binding.platform} to binding {binding.id}')
        return True
    except Exception as e:
...
def notify_user(user, subject, content_lines, action_url=None, action_text='查看详情', priority='normal'):
    ...
    # 2. 通知渠道（按绑定偏好发送）
    try:
        from apps.notifications.models import NotifyBinding
        bindings = NotifyBinding.objects.filter(
            user=user,
            is_active=True,
        ...
        # 将 content_lines 转为纯文本（通知渠道不用 HTML）
        content_str = '\n'.join(
        ...
        for binding in bindings:
            if priority != 'important' and not binding.receive_all:
                continue  # 用户选择仅接收重要通知时跳过
...
def notify_approval_created(flow):
    ...
    approver_list = [
        node.approver for node in flow.approvalnode_set.all()
        if node.approver and node.status == 'pending'
    ]
...
def notify_urged(flow, urged_by):
    ...
    """
    pending_nodes = [
        node for node in flow.approvalnode_set.all()
        if node.status == 'pending' and node.approver
    ]
    system_url = _get_system_url()
