...
## 1b. 侧边栏导航结构（2026-05-28）

系统侧边栏经去冗余重构后，保留 **5个分组标题 + 21个菜单项**：
...
**侧边栏模板**：`templates/core/base.html` / `templates/base.html`
**权限控制**：`core/context_processors.py` 提供 `user_menu_codes`，逐项判断
**模块自注册**：各 `apps/*/modules.py` 调用 `register_module()`，自动计入权限矩阵
- 财务8模块注册在 `apps/finance/modules.py`
- 系统8模块注册在 `apps/core/modules.py`
- 项目/审批/采购/运营/CRM/文件各自在对应 app 的 modules.py
