...
for u in User.objects.filter(is_active=True, is_superuser=False)[:2]:
    ...
    client.logout()
    resp = client.post(
        '/api/core/auth/login/', {'username': u.username, 'password': 'admin123'}, content_type='application/json'
    )
    ...
    if resp.status_code == 200:
        ...
        # 用户授权的公司
        ump_companies = set(
            UserModulePermission.objects.filter(user=u, module__name='invoice').values_list('company_id', flat=True)
        )
