...
from apps.core.models import (
    User, UserModulePermission, UserCompanyPermission,
    Module, ModuleAction, Permission, ACTION_BITS, CompanyRole,
    UserCompanyRole
)
...
ucp_data = list(UserCompanyPermission.objects.all().values(
    'id', 'user_id', 'company_id', 'module_id', 'action_id', 'is_granted'
))
...
def migrate_ucp_to_ump():
    ...
    for (user_id, company_id, module_id), data in grouped.items():
        ...
        ump, created = UserModulePermission.objects.get_or_create(
            ...
            module_id=module_id,
            defaults={'granted_bits': data['bits']}
        )
...
for username, module, expected in test_cases:
    ...
    if user.is_superuser:
        ...
        result = list(UserModulePermission.objects.filter(
            user=user, module__name=module
        ).values_list('company_id', flat=True).distinct())
